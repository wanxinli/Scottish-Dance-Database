...
public class DetailedResult extends JPanel{
	...
	private void recordTable(List<Controller> records){
		...
		for(int i=0; i<records.size(); i++){
			...
			data[i][3] = record.getTypeId();
			data[i][4] = record.getReps();
			data[i][5] = record.getBarsPerRepeat();
			data[i][6] = record.getPlayTime();
		}
...
private void albumInfo(Controller album) {
	...
	//List<Controller> records = Controller.searchByKey(id,"albumsrecodingsmap");
	//Controller media = Media.searchByID(id, "media");
