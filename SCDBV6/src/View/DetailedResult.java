...
//				
				...
				case "recording":
					Controller.mark(id,category);
					List<Controller> collection1 = Controller.getList(id,category);
					for(Controller el: collection1) {
						Controller.mark(""+el.getId(),"tune");						
					}
					SuccessfulMsg.message("The record has been marked successfully");
					break;	
				case "dance":
					Controller.mark(id,category);
					List<Controller> colexion = Publication.getList(id,"dancespublicationsmap");
					for(Controller el: colexion) {
						Controller.mark(""+el.getId(),"publication");
					}
					SuccessfulMsg.message("The record has been marked successfully");
					break;
				}
	...
	private void recordTable(List<Controller> records){
		...
		for(int i=0; i<records.size(); i++){
			...
			data[i][4] = Controller.checkMarked(record.getId(),"recording"); 
...
private void tuneInfo(Controller tu) {
	...
	panel.setAlignmentX(CENTER_ALIGNMENT);
	add(panel);
	if(Controller.checkMarked(tune.getId(), category)=='*')
		flag = true;
}
...
private void albumInfo(Controller album) {
	...
	panel.setAlignmentX(CENTER_ALIGNMENT);
	add(panel);
	List<Controller> records = Controller.getList(id,"album");
	recordTable(records);
