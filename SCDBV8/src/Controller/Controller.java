...
public abstract class Controller {
	...
	public static String getCodeIndex(int id){
		Connection connection = null;
		try {
			Class.forName("org.sqlite.JDBC");
			connection = DriverManager.getConnection("jdbc:sqlite:markeddb.db");
			SqlQuery query = new SqlQuery(connection,"codeTable");
			ResultSet rs = 	query.checkIndexCode(id);
			if(rs.next())
	...
	public static boolean addIndexCode(String id,String indexCode){
		Connection connection = null;
		try {
			Class.forName("org.sqlite.JDBC");
			connection = DriverManager.getConnection("jdbc:sqlite:markeddb.db");
			SqlQuery query = new SqlQuery(connection,"codeTable");
			boolean flag = 	query.indexCodeUpdate(Integer.parseInt(id),indexCode);
			connection.close();
			return flag; 
		}catch (ClassNotFoundException e1) {
			e1.printStackTrace();
